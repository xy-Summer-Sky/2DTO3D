...
impl HSVProcessing {
    ...
    pub fn extract_contours(&self, image: &DynamicImage, color_model: &ColorModel) -> ImageBuffer<Luma<u8>, Vec<u8>> {
        ...
        for (x, y, pixel) in hsv_image.enumerate_pixels() {
            ...
            if h >= color_model.hsv_range[0][0] as f32 && h <= color_model.hsv_range[1][0] as f32 &&
                s >= color_model.hsv_range[0][1] as f32 / 255.0 && s <= color_model.hsv_range[1][1] as f32 / 255.0 &&
                v >= color_model.hsv_range[0][2] as f32 / 255.0 && v <= color_model.hsv_range[1][2] as f32 / 255.0 {
    ...
    pub fn save_contours_as_svg(&self, contours: &ImageBuffer<Luma<u8>, Vec<u8>>, output_path: &Path) {
