...
def genTopFace(contour, child_contours, height):
    ...
    triangles = contourb[tri_simplices]
...
def process_json(json_data):
    parent_contour = [(point['lon'], point['lat']) for point in json_data['parent']['contour_points']]
    child_contours = [[(point['lon'], point['lat']) for point in child['contour_points']] for child in json_data['children']]
    height_params = [json_data['parent']['height']] + [child['height'] for child in json_data['children']]
    contours = np.array(parent_contour + [point for child in child_contours for point in child])
    save_path = "Building2-模型.obj"
    # 将 parent_contour 和 child_contours 转换为 NumPy 数组
    parent_contour = np.array(parent_contour)
    child_contours = [np.array(child) for child in child_contours]


    # 将第一个点追加到末尾，形成闭合轮廓
    parent_contour = np.vstack([parent_contour, parent_contour[0]])
    child_contours = [np.vstack([child, child[0]]) for child in child_contours]
    contours = np.vstack([parent_contour] + child_contours)
    return contours, parent_contour, child_contours, height_params, save_path
def main():
    with open ('newoutput2.json','r') as f:
        json_data = json.load(f)
    contour,parent_contour,child_contours,height_params,save_path = process_json(json_data)
