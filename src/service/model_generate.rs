...
impl ModelGenerate {
    ...
    fn gen_contour_vertex(&mut self, contour: &Vec<(f64, f64)>, height: f64) {
        for point in contour {
            let lon = point.0;
            let lat = point.1;
            self.v_content.push_str(&format!("\nv {} {} {}", lon, height, lat));
            self.vertex2num_dict.insert(format!("v {} {} {}", lon, height, lat), self.vertex_count);
            self.vertex_count += 1;
        }
    }
    ...
    fn gen_side_face(&mut self, contour: &Vec<(f64, f64)>, height_self: f64, height_target: f64) {
        for i in 1..=contour.len() {
            ...
            let point3 = if i < contour.len() { contour[i] } else { contour[0] };
            let point4 = if i < contour.len() { contour[i] } else { contour[0] };
            let a = (0.0 - height_self) * (point3.1 - point1.1) - (0.0 - height_self) * (point2.1 - point1.1);
            let b = 0.0;
            let c = (point2.0 - point1.0) * (0.0 - height_self) - (point3.0 - point1.0) * (0.0 - height_self);
            ...
            self.side_face_content.push_str(&format!("\nf {} {} {} {}", p1_num, p2_num, p3_num, p4_num));
        }
    }

    pub fn gen_top_face(&mut self, contour: &Vec<(f64, f64)>, child_contours: &Vec<Vec<(f64, f64)>>, height: f64) {
        let mut points: Vec<delaunator::Point> = contour.iter().map(|&(x, y)| delaunator::Point {x, y }).collect();
        ...
        for child_contour in child_contours {
            let path: Vec<Point2<f64>> = child_contour.iter().map(|&(x, y)| Point2::new(x, y)).collect();
            contour_paths.push(path);
        }

        let contour_path2: Vec<Point2<f64>> = contour.iter().map(|&(x, y)| Point2::new(x, y)).collect();

        let tri_centers: Vec<Point2<f64>> = result.triangles.chunks(3).map(|tri| {
            let p1 = &points[tri[0]];
            let p2 = &points[tri[1]];
            let p3 = &points[tri[2]];
    ...
    pub fn gen_top_face2(&mut self, contour: &Vec<(f64, f64)>, height: f64) {
        ...
        let points: Vec<DelaunatorPoint> = contour.iter().map(|&(x, y)| DelaunatorPoint { x, y }).collect();
        ...
        let polygon: Polygon<f64> = Polygon::new(
            LineString::from(
                contour.iter().map(|&(x, y)| Coord { x, y }).collect::<Vec<_>>()
            ),
            vec![]
        );

        // 计算三角形质心并筛选内部的三角形
        let triangles_centers = delaunay.triangles.iter().tuples().map(|(&i1, &i2, &i3)| {
            let p1 = &points[i1];
            let p2 = &points[i2];
            let p3 = &points[i3];
        ...
        // 过滤出所有中心点在多边形内的三角形
        let is_inside = triangles_centers.iter()
    ...
    pub fn gen_bottom_face(&mut self, contour: &[(f64, f64)], height: f64) {
        ...
        use geo_types::Coord;
        use itertools::Itertools;
        let points: Vec<DelaunatorPoint> = contour.iter()
        ...
        // 计算三角形中心
        let centers = delaunay.triangles.iter().tuples().map(|(&i1, &i2, &i3)| {
            let c = vec![i1, i2, i3].iter().map(|&i| &points[i]).fold(
                DelaunatorPoint { x: 0.0, y: 0.0 },
                |acc, p| DelaunatorPoint { x: acc.x + p.x / 3.0, y: acc.y + p.y / 3.0 }
            );
            point!(x: c.x, y: c.y)
        }).collect::<Vec<_>>();

        // 筛选出中心在多边形内的三角形索引
        let is_inside = centers.iter()
            .map(|center| polygon.contains(center))
            .collect::<Vec<bool>>();
        ...
        for (i, &inside) in is_inside.iter().enumerate() {
            if inside {
                ...
                for &index in indices.iter().rev() { // 注意逆序处理
                    let point = &points[index];
                    let substring = format!("v {} {} {}", point.x, height, point.y);
                    if let Some(&num) = self.vertex2num_dict.get(&substring) {
    ...
    fn generate_model(&mut self, contour: &Vec<(f64, f64)>, parent_contour: &Vec<(f64, f64)>, child_contours: &Vec<Vec<(f64, f64)>>, height_params: &Vec<f64>) {
    ...
    fn process_json(&self,json_data: &serde_json::Value) -> (Vec<(f64, f64)>, Vec<(f64, f64)>, Vec<Vec<(f64, f64)>>, Vec<f64>, String) {
        ...
        let height_params: Vec<f64> = std::iter::once(json_data["parent"]["height"].as_f64().unwrap())
            .chain(
                json_data["children"]
                    .as_array()
                    .unwrap()
            ...
            .collect();
        let contours: Vec<(f64, f64)> = parent_contour.iter().chain(child_contours.iter().flatten()).cloned().collect();
        let save_path = "Building2-模型.obj".to_string();

        (contours, parent_contour.clone(), child_contours, height_params, save_path)
    }
    pub fn model_generate_api(&mut self, inputfile:&str, outputpath:&str)
    {
            ...
            let (contour, parent_contour, child_contours, height_params, save_path) = self.process_json(&json_data);
            self.generate_model(&contour, &parent_contour, &child_contours, &height_params);
            let obj_content = format!("{}{}{}{}{}{}{}{}",
                                  self.title_content, self.v_content, self.vt_content, self.vn_content, self.face_content, self.top_face_content, self.bottom_face_content, self.side_face_content);
            println!("{}", obj_content);
            let mut obj_file = File::create(save_path).expect("Unable to create file");
