...
def process_image_from_json(json_data):
    global g_image_original, g_image_zoom, g_image_show, g_location_win, g_zoom, g_step, g_window_wh
    ...
    mouse(cv2.EVENT_LBUTTONDOWN, g_location_win[0], g_location_win[1], None, None)
    mouse(cv2.EVENT_RBUTTONDOWN, g_location_win[0], g_location_win[1], None, None)
    mouse(cv2.EVENT_MOUSEMOVE, g_location_win[0], g_location_win[1], cv2.EVENT_FLAG_LBUTTON, None)
    mouse(cv2.EVENT_MOUSEWHEEL, g_location_win[0], g_location_win[1], 1, None)

    # Save the processed image
    cv2.imwrite("processed_image.png", g_image_show)
...
if __name__ == "__main__":
    ...
    json_path = os.path.join("..", "..", "..","assets", "images", "contours", "input.json")
